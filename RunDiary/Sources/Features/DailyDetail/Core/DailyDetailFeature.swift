  ...
  var body: some Reducer<State, Action> {
    Reduce { state, action in
      ...
      case .weekChanged(let offset):
        ...
        if let selectedDateWeekday = Calendar.current.dateComponents(
          [.weekday], from: state.selectedDate
        ).weekday,
      ...
      case .recordFetchedSuccess(let record):
        state.isLoading = false
        state.runningRecord = record
        state.errorMessage = nil
        return .none

      case .recordFetchedFailure(let errorMessage):
        state.isLoading = false
